...
class NumberInputModal(ui.Modal):
    ...
    async def on_submit(self, interaction: Interaction[ClientContext]) -> None:
        ...
        if extra < self.min_val or extra > self.max_val:
            await interaction.response.send_message(f"Pick a number between {self.min_val} and {self.max_val}.", ephemeral=True,
                                                    delete_after=5)
            return
...
class ResistModal(NumberInputModal):
    def __init__(self, record: MessageRecord, *, min_val: int = 0, max_val: int = 99):
        if isinstance(record.roll_result, ResistableRoll):
            ...
            self.threshold = 0
        super().__init__(title=f"Resist {record.label} ({self.threshold})?",
    ...
    async def on_resist_confirm(self, interaction: Interaction[ClientContext], dice: int):
        context = InteractionContext(interaction)
        from chance_sprite.roll_types.basic import ThresholdRoll, roll_simple
                
        threshold_roll: ThresholdRoll = roll_simple(dice=dice, threshold=self.threshold, limit=0)
        self.record = context.message_store.get(self.record.message_id)
        await context.transmit_result(f"Resisting {self.record.label} ({self.threshold})", threshold_roll)

    async def after_use(self, interaction: Interaction[ClientContext]):
        context = InteractionContext(interaction)
