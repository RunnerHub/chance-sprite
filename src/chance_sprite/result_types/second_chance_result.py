...
class SecondChanceHitsResult(HitsResult):
    ...
    def get_dice_mask(self):
        all_dice_for_mask = self.rolls[:self.dice] + self.rerolled_dice[:self.dice - self.dice_hits]
    ...
    def render_roll(self, context: InteractionContext):
        ...
        line += f"\n+{context.emoji_manager.packs.reroll}" + self.render_rerolls(
            context) + self.render_limited_rerolled_hits()
        return line
    ...
    def render_rerolls(self, context: InteractionContext) -> str:
        ...
        dice_emojis = [
            chosen_emojis[x - 1] if not reroll_mask or reroll_mask[i] else limited_emojis[x - 1]
        ...
        if post_dice_adjustment < 0:
            ...
            line += "-~~" + "".join(str(x) for x in baleeted_dice[:n_original_rerolls]) + "~~"
        else:
