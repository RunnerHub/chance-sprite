...
from chance_sprite.fungen import RollMeta, build_discord_callback, invoke_roll_and_transmit
...
def iter_modules(package_name: str) -> list[ModuleType]:
    ...
    return [importlib.import_module(m.name) for m in pkgutil.walk_packages(pkg.__path__, pkg.__name__ + ".")]
...
def identity(fn: RollFunc, base_package: str) -> tuple[str | None, str, str]:
    ...
    group = meta.group if meta and meta.group is not None else derive_group_from_module(fn.__module__, base_package)
...
async def setup(bot: ClientContext) -> None:
    root = app_commands.Group(name=bot.base_command_name, description="SR5 d6 dice rolling tools.")
    ...
    for fn in roll_funcs:
        ...
        cmd = app_commands.Command(name=cmd_name, description=cmd_desc, callback=callback)
        ...
        if group_name is None:
            ...
            if g is None:
                g = app_commands.Group(name=group_name, description=f"{group_name.title()} commands")
                root.add_command(g)
                subgroups[group_name] = g
            g.add_command(cmd)

    bot.tree.add_command(root)
    log.info("Registered /%s with %d roll command(s) across %d subgroup(s)", root.name, len(roll_funcs), len(subgroups))
